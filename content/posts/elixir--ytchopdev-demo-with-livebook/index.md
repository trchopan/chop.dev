...
Mình thường xuyên xem các video về công nghệ và kiến thức trên Youtube. Một ngày chắc tiêu thụ tầm 5 tới 10 videos khoản 1~2 tiếng 1 ngày. Tổng cả ăn ngủ thì 2 tiếng cũng là gần 10% thời gian của ngày. Thay vì xem phim hay facebook Tiktok thì mình xem Video kiến thức.
...
Nếu là mới xem video về vật lý hay công nghệ thì giở Google ra search các thông tin liên quan mà chưa hiểu để xem mình hiểu đúng không hay video chém thông tin đúng không, có thông tin trái chiều nào không.

Vòng lặp xem rồi thực hành cứ vậy mà nạp vào đầu rồi có cơ hội ứng dụng hay chia sẻ bàn luận với bạn bè thì nó thành kinh nghiệm thực tế.
...
Ở đây đang không chê content tiếng Việt nhe, cộng đồng Youtuber Việt Nam cũng làm content hay lắm. Ý mình là mở rộng được thêm, và có thông tin nhanh và nhiều hơn thôi.
...
Chương trình này nhận vào một đường dẫn Youtube và dịch tự động thành tiếng Việt và lồng âm vào video luôn.
...
Một bảng dịch mất khoảng 5 phút. Và vì xài AI và máy chủ để dịch tự động nên tốn nhiều chi phí mình có giới hạn số lượng dịch bằng cách xếp hàng các bản dịch và chỉ được xếp tối đa 10 video.
...
Mình sẽ không đi full 100% chương trình được ha vì để dựng lên website chương trình như mới demo thì còn rất rất nhiều thứ xung quanh như lập trình webserver, UI rồi quản lý deployment quản lý server đồ nữa.
...
Ok tiếp theo mình sẽ chọn 1 video để dịch. Video nào đây video nào mà có transcript là dịch được. Video nào ngắn để demo thôi tầm 5 phút.

Mình rất thích video của Ted-Ed nên sẽ pick video này ha "Should you care what your parents think?" - "Bạn có nên quan tâm đến suy nghĩ của bố mẹ không?". Mình đang làm ba của 1 đứa 5 tuổi và chuẩn bị đón 1 đứa nữa về nên mấy video tâm lý học này rất thú vị. 

Rồi giờ chúng ta sẽ mở phần Transcript ra bằng cách nhấn vào nút Description và vào transcript ở đây.

Xong bật Browser dev lên target vào element này. Đây là element transcript của Youtube. Nếu các bạn để ý trong này là toàn bộ thông tin của transcript cùng với timestamp của từng đoạn thoại.

OK, mình chỉ cần click để target vào element segments-container này rồi qua tab Console để làm 1 thao tác nữa. Clean up các HTML xung quanh để được đoạn text transcript thôi. Dễ xử lý.
...
Đầu tiên chúng ta phải tạo ra module để gói các function lại. Function trong Elixir luôn phải nằm trong 1 module.

Chúng ta sẽ tạo module DemoYt bằng defmodule DemoYt như vầy ha.

OK nhìn lại format của transcript_raw các bạn để ý format sẽ là 1 dòng timestamp 1 dòng content. Giờ chúng ta sẽ parse nó bằng Elixir như thế nào đây.

Function parse_transcript(raw).
...
Tức là làm theo từng bước từng bước. Dễ nhìn dễ hiểu. Ở đây ai muốn debug thì dùng function tap ở giữa để lấy kết quả ha. Chúng ta có thể tap để print out. Trong Elixir thì có hàm IO.inspect để in giá trị ra standard output.
...
Ra các string còn lại sau khi đã filter timestamp ha.
...
```
translated = AI.translate_transcript("Hello, this is Chop")
```
...
Đối với combine_sentences chúng ta sẽ pipe result thành 1 array để xử lý từng cụm một. Việc này cũng góp phần cải thiện chất lượng công đoạn chuyển văn bản thành giọng nói.

Vậy là có 2 function để process transcript. Bước 1 là combine xong translate. Chúng ta sẽ làm 1 hàm translate_transcript chung để xử lý input transcript.
...
ElevenLabs thì chất lượng đọc tốt hơn nhiều nhưng chi phí thì cũng cao hơn nhiều luôn. Tận gấp đôi gấp 3. Mình cũng có 1 cái Studio kia sponsor cho chi phí ElevenLab để dịch cho họ, nhưng chương trình public này thì mình phải giới hạn chi phí nên xài Google TTS thôi.
...
Hàm này đầu tiên loop qua các sentence với enumerate map. Mỗi sentence chạy hàm text_to_speech ở trên.
...
Và câu trả lời như các bạn thấy có nhiều cách. Đến đây thì cũng chia sẽ bây giờ mình lập trình chủ yếu sử dụng ChatGPT liên tục, dùng nó để gởi ý các cách thức xử lý vấn đề để mà áp dụng.

Thực ra cái nghiệp lập trình là xử lý tình huống bằng code thôi, chủ yếu là lên mạng lần mò. Có con GPT này thì quá tiện rồi gợi ý cách xử lý giùm.

Với các gợi ý thì mình sẽ kết hợp với kinh nghiệm để đưa ra giải pháp. Nhanh hơn nhiều việc Google rồi đọc các câu trả lời rời rạc trên Stackoverflow hay các Forum.

Cũng lưu ý là ý mình không phải là chê Google hay Stackoverflow nha. Các tình huống chuyên sâu hay phức tạp thì cũng sẽ phải tự mần mò trong các câu trả lời của các chuyên gia thôi. Nhưng với các trường hợp độ khó thấp trung bình thì GPT là giải pháp tốt.

OK quay lại giải pháp nối file thì mình sẽ sử dụng cách "file list txt". Đơn giản là list ra đường dẫn đến các file audio cần xử lý trong 1 file danh sách rồi dùng lệnh ffmpeg để chạy.
...
OK và như vậy có thể chạy lệnh để process text to spech, combine audio files rồi.
...
Thì đúng rồi, tại dịch và chuyển văn bản thành nói đâu biết gì về video đâu nên ghép vào sẽ bị lệch.

Để xử lý tình huống lệch thì muôn vàng vấn đề, mình đã giải quyết 1 phần lớn ở trong dịch vụ Yt trên trang chop.dev của mình. Các bạn có thể thử dịch và thấy audio được khớp với các timestamp để đạt khớp lời thoại với hình ảnh đang được diễn đạt nhất có thể.

Cũng chưa hoàn chỉnh lắm tại ngôn ngữ khác nhau nên câu dài ngắn khác nhau. Mình phải tăng tốc độ đọc để ghép vô, và đôi khi bị overlap giữa các câu. Tương lai thì mình sẽ cải tiến thêm.

Còn với lập trình mẫu trên notebook này thì mình sẽ demo tác vụ mà mình tính toán phần start của audio và tốc độ đọc để khớp khớp với video tàm tạm ha.
...
Và mục tiêu của chúng ta là tính toán là làm sao khớp được audio của chúng ta ngay sau khi intro.

![](./elixir--ytchopdev-demo-with-livebook-6.jpg)

Đoạn Intro là không có lời thoại ha. Thường là nhạc và các hình ảnh bắt mắt để nhớ tới thương hiệu thôi. 

Chungs ta sẽ cần tính toán 2 thứ để khớp audio với các phần chính của video.

Đó là thời gian delay audio bao nhiêu để khớp với phần đầu chính đầu tiên của video.

Và tốc độ đọc của audio là bao nhiêu để khớp điểm kết thúc của audio với thời gian kết thúc của video.
...
OK có start time rồi thì chúng ta cần xác định độ dài của audio và độ dài của video xem tỷ lệ của chúng là bao nhiêu. Ví dụ:
...
```
  def calculate_audio_tempo(video_file, audio_file, start_delay) do
    ...
    (video_length - start_delay) / audio_length
  end
``` 

Công thức cũng đơn giản ha

audio tempo = (video length - delay) / audio length
